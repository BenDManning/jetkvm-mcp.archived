...
| `jetkvm-mcp-fixture-runner` | O, M, P/H, or P/network according to its prescribed MCP calls | Executes at most 256 plan calls in sequential batches of at most eight concurrent calls, each with a 1–60 second deadline. Any call not declared closed-world read-only requires an explicit consequence acknowledgement and per-run owner authorization. Output retains only ordinals, reviewed tool names, and allowlisted stable outcomes; arguments, results, images, child diagnostics, and free-form errors are discarded. |
