...
| B8 — source to delivered artifact | Repository changes, Go modules, CI actions, build helpers, base images, archives, checksums, and containers | Tests, module checksums, vulnerability analysis, immutable CI Action and container-image pins, explicit Buildx selection, non-root container execution, native and container checksums/notices/SPDX SBOMs, and a non-publishing keyless signature/provenance rehearsal provide bounded evidence. The integrated protected-tag workflow stages and verifies all subjects before immutable version publication and moves only `latest` last. No reproducible-build comparison is claimed. |
...
| Device names, URLs, origins, Wake-on-LAN targets, addresses | Operational configuration, not credentials. They exist in configuration/runtime state and selected MCP inputs/results; device/target names can also appear in caller-visible tool or local debug diagnostics. Device URL path prefixes are retained, while URL credentials are rejected. |
...
| `jetkvm_capture_screen` | O (high confidentiality) | Lazily acquires or reuses the managed generation, copies one fresh post-admission frame, decodes it locally after releasing the generation lease, and returns a PNG to the MCP caller. Any visible host secret can be captured. |
...
| `jetkvm-mcp-validate` | O (bounded) | Lists tools, validates that safe configured-device discovery contains the selected alias, and performs 1–100 status, dedicated redacted virtual-media-status, and capture/full-PNG-decode sequences. It never calls keyboard, mouse, media mutation, power, wake, or raw RPC and emits one fixed-shape sanitized pass/fail report. |
| `jetkvm-mcp-fixture-runner` | O, M, P/H, or P/network according to its prescribed MCP calls | Executes at most 256 plan calls in sequential batches of at most eight concurrent calls, each with a 1–60 second deadline. Any call not declared closed-world read-only requires an explicit consequence acknowledgement under the standing fixture authorization. Output retains only ordinals, reviewed tool names, and allowlisted stable outcomes; arguments, results, images, child diagnostics, and free-form errors are discarded. |
...
| `connected`, `applicationVersion`, `systemVersion`, `activeExtension`, `atxPowerOn`, `dcPowerOn`, `dcVoltage`, `videoReady`, `videoWidth`, `videoHeight`, `videoFPS`, `usbState`, `usbWakeAttached`, `warnings` | Observed appliance/host state returned to the MCP caller; not persisted or application-logged. |
...
| Tool error content | Caller-visible operational diagnostics. Typed execution paths return a versioned JSON taxonomy (`code`, `outcome`, `retryable`) and discard raw firmware messages/bodies. `outcome: unknown` means a mutation may have been sent and is never retryable; `not_sent` is used only when dispatch is known not to have begun. All MCP input-schema failures return the same value-free `invalid_input` result without reflecting rejected values. The application does not separately log or persist tool errors. |

### Configuration, environment, CLI, and validator fields

| Surface | Classification and control |
|---|---|
| `devices` map key | Becomes the runtime `device` identifier. Names are ordinary operational identifiers. |
...
| Validator `--binary`, `--config`, `--device` | Local inputs omitted from the bounded JSON report. Child stderr and detailed failures are discarded; output contains only check names and capture dimensions/size. |
...
and `debug rpc` returns raw output to its invoking terminal. All MCP input-schema
failures instead return value-free errors without reflecting rejected arguments.
Explicit output destinations can persist data independently; the caller/operator is responsible
...
| T-05 | Privacy disclosure through logs, diagnostics, MCP output, validation evidence, or client retention can expose typed text, host screens, local paths, URL credentials, or raw RPC payloads. | **Implemented control:** stdout/stderr separation, no request logger/database, bounded typed outputs, value-free YAML decode and MCP input-schema errors, discarded invalid-output detail, bounded validator JSON, discarded child stderr, image-buffer clearing in the validator, and secret/path regression tests. See [main.go](../cmd/jetkvm-mcp/main.go), [main.go](../cmd/jetkvm-mcp-validate/main.go), [main_test.go](../cmd/jetkvm-mcp-validate/main_test.go), [typed_results_test.go](../internal/mcpserver/typed_results_test.go), and [privacy_test.go](../internal/config/privacy_test.go). | **Deployment requirement:** configure MCP hosts, proxies, terminals, shells, and crash handling for the desired retention of explicit outputs, and remove actual secrets from issue/support evidence. **Not implemented:** centralized redaction middleware, encrypted audit storage, client-side retention control, or full memory zeroization. Explicit successful MCP results and debug outputs remain visible to their trusted caller. |
| T-06 | CPU, memory, file, subprocess, connection, or appliance exhaustion can deny service. | **Implemented control:** strict bounded global/per-device operation, connection-attempt, capture, and decoder admission; context-cancelable per-device mutation serialization; 1 MiB configuration, MCP request-body, and HTTP-response limits; 16 KiB RPC frames; 64 KiB signaling frames; 64 pending RPCs; bounded H.264/PNG/FFmpeg buffers; decoder and network deadlines; fixed image dimensions; independent five-second HTTP header, 15-second MCP body-read, and 60-second idle limits; and cancellation propagation. Capacity exhaustion is an immediate `busy`/`not_sent` result, not a queue. See [manager.go](../internal/jetkvm/manager.go), [transport.go](../internal/mcpserver/transport.go), [main.go](../cmd/jetkvm-mcp/main.go), [http_integration_test.go](../cmd/jetkvm-mcp/http_integration_test.go), [rpc_session.go](../internal/jetkvm/rpc_session.go), [rpc_codec.go](../internal/jetkvm/rpc_codec.go), [video.go](../internal/jetkvm/video.go), [protocol_version.go](../internal/mcpserver/protocol_version.go), and [decoder_ffmpeg.go](../internal/jetkvm/decoder_ffmpeg.go). | **Deployment requirement:** restrict client connection rates and concurrency and constrain process CPU, memory, PIDs, files, and network externally; size media roots deliberately. **Not implemented:** per-principal quotas, policy/grant engines, distributed queues, circuit breakers, or public-edge rate limiting. Repeated admitted captures and 64 GiB uploads remain expensive. |
...
| T-12 | Build compromise through source, dependency, CI action, base image, or release control can ship a binary with all server authority. | **Implemented control:** `go.sum`/`go mod verify`, race/tests/vet/vulnerability analysis, full-SHA CI Actions, checksum-verified tools, digest-pinned QEMU, BuildKit, frontend, builder, base, and runtime images, a minimal non-root runtime, and reviewed static/release tool contracts. The integrated workflow admits only read-only pull-request staging, a non-publishing `main` rehearsal, or an exact protected annotated tag at a green `main` commit. It cachelessly builds and verifies both Linux archives and the multi-platform container before the selected release-stage job signs and attests every supported subject. Only the tag-selected publisher can write a release or package. Publication consumes the protected version with a draft, verifies the exact GHCR digest, makes the GitHub Release immutable, and moves only `latest` last. See [ci.yml](../.github/workflows/ci.yml), [release.yml](../.github/workflows/release.yml), [Dockerfile](../Dockerfile), [publish-release.sh](../scripts/publish-release.sh), [go.mod](../go.mod), [tools/go.mod](../tools/go.mod), [.goreleaser.yaml](../.goreleaser.yaml), and [dependency-policy.md](dependency-policy.md). | **Deployment requirement:** consume artifacts from the canonical release record, run the documented repository/workflow/tag/commit-constrained checksum/signature/SBOM/provenance verification, review dependency/action updates, and protect GitHub/release credentials. **Not implemented:** reproducible-build comparison or defense against a compromised reviewed source, runner, dependency publisher, or repository owner. Checksums, attestations, and pinning authenticate the recorded build; they do not prove the source benign. |
