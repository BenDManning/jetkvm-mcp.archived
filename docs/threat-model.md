...
| B8 — source to delivered artifact | Repository changes, Go modules, CI actions, build helpers, base images, archives, checksums, and containers | Tests, module checksums, vulnerability analysis, immutable CI Action and container-image pins, explicit Buildx selection, non-root container execution, native archive checksums/notices/SPDX SBOMs, and a non-publishing keyless signature/provenance rehearsal provide bounded evidence. Container release subjects and integrated immutable publication remain separate work. No reproducible-build comparison is claimed. |
...
| T-12 | Build compromise through source, dependency, CI action, base image, or release control can ship a binary with all server authority. | **Implemented control:** `go.sum`/`go mod verify`, race/tests/vet/vulnerability analysis, full-SHA CI Actions, a checksum-verified Buildx binary, digest-pinned QEMU, BuildKit, frontend, builder, and runtime images, a minimal non-root runtime, and reviewed static/release tool contracts. The native release snapshot admits only Linux amd64/arm64, packages a checked exact dependency notice inventory, generates archive-specific SPDX SBOMs and SHA-256 checksums, and verifies embedded version/commit/toolchain. A non-publishing hosted rehearsal uses short-lived OIDC for a keyless checksum bundle and GitHub provenance constrained to its repository/workflow/ref/commit. See [ci.yml](../.github/workflows/ci.yml), [native-release.yml](../.github/workflows/native-release.yml), [Dockerfile](../Dockerfile), [go.mod](../go.mod), [tools/go.mod](../tools/go.mod), [.goreleaser.yaml](../.goreleaser.yaml), and [dependency-policy.md](dependency-policy.md). | **Deployment requirement:** consume artifacts from the canonical release record, run the documented identity-constrained checksum/signature/SBOM/provenance verification, review dependency/action updates, and protect GitHub/release credentials. **Not implemented:** the container release-subject path, integrated protected-tag immutable publication, or reproducible-build comparison. Checksums and pinning alone do not defend against a compromised source or publisher. |
