...
|---|---|---|
| `jetkvm_get_status` | O | Reads connectivity, firmware/application versions, power/video/USB/extension state, warnings, and a bounded raw `virtualMedia` state string. It does not prove that a later mutation is safe. |
...
| `jetkvm_virtual_media` | O or M (destructive hint) | `status` observes; `mount_url` makes the appliance fetch and mount HTTP(S); `upload` reads and stores a confined local file; `mount_file` uploads then mounts; `unmount` changes media state. Upload cleanup cannot prove what firmware persisted after a lost acknowledgement. |
...
| `operation`, `key`, `modifiers`, `button`, `x`, `y`, `dx`, `dy`, `wheel_x`, `wheel_y` | Host-control intent. Values are schema/handler bounded, then translated to HID or media calls. Treat traces containing them as private operational data. |
...
| `source` | For file operations, a relative local media path confined below `media_directory`; for URL mount, an HTTP(S) URL sent to firmware. Local paths and URL secrets must never be logged or persisted. Query-string secrets are not rejected, so they must not be supplied. |
...
| `virtualMedia` | Bounded raw firmware JSON represented as a string in status. It is returned to the trusted caller but must not be copied into logs, diagnostics, issue reports, or retained validation evidence. |
