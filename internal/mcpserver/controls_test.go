...
func TestKeyboardAndMouseSchemasEnforceOperationSpecificArguments(t *testing.T) {
	tests := []struct {
		...
		{name: "keyboard type text rejects multibyte text", schema: keyboardSchema(), input: map[string]any{"device": "lab", "operation": "type_text", "text": strings.Repeat("é", 4096)}},
		{name: "keyboard type text rejects unsupported control byte", schema: keyboardSchema(), input: map[string]any{"device": "lab", "operation": "type_text", "text": "hello\x00"}},
		...
		{name: "keyboard press key rejects ASCII whitespace", schema: keyboardSchema(), input: map[string]any{"device": "lab", "operation": "press_key", "key": " 	\n"}},
		{name: "keyboard press key rejects Unicode whitespace", schema: keyboardSchema(), input: map[string]any{"device": "lab", "operation": "press_key", "key": "\u2003"}},
		...
		{name: "mouse move absolute rejects null x", schema: mouseSchema(), input: map[string]any{"device": "lab", "operation": "move_absolute", "x": nil, "y": 1}},
		...
		{name: "mouse move relative rejects null dy", schema: mouseSchema(), input: map[string]any{"device": "lab", "operation": "move_relative", "dx": 1, "dy": nil}},
...
func TestMalformedControlCallsAreSchemaRejectedBeforeProviderDispatch(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: KeyboardToolName, arguments: map[string]any{"device": "lab", "operation": "type_text", "text": strings.Repeat("é", 4096)}},
		{name: KeyboardToolName, arguments: map[string]any{"device": "lab", "operation": "press_key", "key": "enter", "text": "private-argument"}},
		{name: KeyboardToolName, arguments: map[string]any{"device": "lab", "operation": "press_key", "key": "\u2003"}},
		{name: MouseToolName, arguments: map[string]any{"device": "lab", "operation": "move_absolute", "x": 32768, "y": 1}},
		{name: MouseToolName, arguments: map[string]any{"device": "lab", "operation": "move_absolute", "x": nil, "y": 1}},
